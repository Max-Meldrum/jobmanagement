...
/** ExecutionEnvironment is used by Executors. It creates an
  * "isolated" environment for each App, where it can store binaries,
  * log files, and configuration files.
  */
private[executor] class ExecutionEnvironment(appId: String) extends LazyLogging {
